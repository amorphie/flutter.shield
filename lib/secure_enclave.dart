...
export 'src/models/secure_enclave_log_data.dart';

class SecureEnclave implements SecureEnclaveBase {
  @override
  Future<void> Function(SecureEnclaveLogData logData)? get log => SecureEnclavePlatform.instance.log;

  @override
  set log(Future<void> Function(SecureEnclaveLogData logData)? fn) {
    SecureEnclavePlatform.instance.log = fn;
  }

  @override
  Future<ResultModel<String?>> decrypt({required String tag, required Uint8List message}) {
    return SecureEnclavePlatform.instance.decrypt(tag: tag, message: message);
  }

  @override
  Future<ResultModel<Uint8List?>> encrypt({required String tag, required String message}) {
    return SecureEnclavePlatform.instance.encrypt(tag: tag, message: message);
  }

  @override
  Future<ResultModel<bool>> storeServerPrivateKey({required String tag, required Uint8List privateKeyData}) {
    return SecureEnclavePlatform.instance.storeServerPrivateKey(tag: tag, privateKeyData: privateKeyData);
  }

  @override
  Future<ResultModel<String?>> getServerKey({required String tag}) {
    return SecureEnclavePlatform.instance.getServerKey(tag: tag);
  }
  ...
  @override
  Future<ResultModel<String?>> sign({required String tag, required Uint8List message}) {
    ...
    return SecureEnclavePlatform.instance.sign(tag: tag, message: hashBytes);
  }

  @override
  Future<ResultModel<bool?>> verify({required String tag, required String plainText, required String signature}) {
    return SecureEnclavePlatform.instance.verify(tag: tag, plainText: plainText, signature: signature);
  }
